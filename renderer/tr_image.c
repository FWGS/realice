...
===============
*/
void Upload32( unsigned *data, int width, int height, qboolean mipmap, qboolean picmip, qboolean lightMap, qboolean force32bit, qboolean has_alpha, int *format, int *pUploadWidth, int *pUploadHeight )
{
	...
	if( !has_alpha )
	{
		if( glConfig.textureCompression == TC_S3TC && !lightMap )
			internalFormat = GL_RGB4_S3TC;
		else if( r_texturebits->integer == 16 )
			internalFormat = force32bit ? 3 : GL_RGB5;
			...
			internalFormat = 3; // the hell is 3?
	}
	else
	{
		if( r_texturebits->integer == 16 )
			internalFormat = force32bit ? GL_RGBA8 : GL_RGBA4;
		else if( r_texturebits->integer == 32 )
	...
	if( mipmap )
	{
		...
		while( scaled_width > 1 || scaled_height > 1 )
		{
			...
			if( r_colorMipLevels->integer )
				R_BlendOverTexture((byte *)data, scaled_width * scaled_height, mipBlendColors[miplevel] );
...
================
*/
image_t *R_CreateImage( const char *name, const byte *pic, int width, int height, qboolean mipmap, qboolean allowPicmip, qboolean force32bit, int has_alpha, int glWrapClampMode )
{
	image_t  *image;
	...
	if( !strncmp( name, "*lightmapD", 10 ))
		isDynamic = qtrue;

	if( tr.numImages == MAX_DRAWIMAGES )
		ri.Error( ERR_DROP, "R_CreateImage: MAX_DRAWIMAGES hit\n" );

	image = &tr.images[tr.numImages];
	image->texnum = 1024 + tr.numImages;
	tr.numImages++;

	image->mipmap = mipmap;
	image->allowPicmip = allowPicmip;
	image->dynamicLightmap = isDynamic;
	...
	Upload32((unsigned *)pic, image->width, image->height, image->mipmap,
		 allowPicmip, image->dynamicLightmap, force32bit, has_alpha,
	...
	image->sequencenumber = r_sequencenumber;
...
==============
*/
image_t *R_FindImageFile( const char *name, qboolean mipmap, qboolean allowPicmip, qboolean force32bit, int glWrapClampMode )
{
	image_t *image;
	int     width, height;
	byte    *pic;
	...
	for( image = hashTable[hash]; image; image = image->next )
	{
		if( !strcmp( name, image->imgName ))
		{
			...
			// prolong registration of this image for next map?
			if( r_registration_active )
			{
	...
	// FAKK2 doesn't have this code, but it doesn't need when everything is in PK3
	{
	...
	image = R_CreateImage( name, pic, width, height, mipmap, allowPicmip, force32bit, has_alpha, glWrapClampMode );
	ri.Free( pic );
	return image;
}
...
static void R_CreateDlightImage( void )
{
	...
	tr.dlightImage = R_CreateImage( "*dlight", (byte *)data, DLIGHT_SIZE, DLIGHT_SIZE, qfalse, qfalse, qfalse, qfalse, GL_CLAMP );
	tr.dlightImage->sequencenumber = -1;
}
...
static void R_CreateFogImage( void )
{
	...
	// what we want.
	tr.fogImage = R_CreateImage( "*fog", (byte *)data, FOG_S, FOG_T, qfalse, qfalse, qfalse, qtrue, GL_CLAMP );
	tr.fogImage->sequencenumber = -1;
...
static void R_CreateDefaultImage( void )
{
	...
	tr.defaultImage = R_CreateImage( "*default", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE, qtrue, qfalse, qfalse, qfalse, GL_REPEAT );
	tr.defaultImage->sequencenumber = -1;
}
...
void R_CreateBuiltinImages( void )
{
	...
	tr.whiteImage = R_CreateImage( "*white", (byte *)data, 8, 8, qfalse, qfalse, qfalse, qfalse, GL_REPEAT );
	tr.whiteImage->sequencenumber = -1;
	...
	tr.identityLightImage = R_CreateImage( "*identityLight", (byte *)data, 8, 8, qfalse, qfalse, qfalse, qfalse, GL_REPEAT );
	tr.identityLightImage->sequencenumber = -1;

	tr.scratchImage = R_CreateImage( "*scratch", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE, qfalse, qtrue, qfalse, qfalse, GL_CLAMP );
	tr.scratchImage->sequencenumber = -1;
...
void R_FreeUnusedImages( void )
{
	for( int i = 0; i < tr.numImages; i++ )
	{
		if( tr.images[i].imgName[0] && tr.images[i].sequencenumber != r_sequencenumber && tr.images[i].sequencenumber != -1 )
			R_FreeImage( &tr.images[i] );
	}
}
...
qhandle_t RE_RegisterSkin( const char *name )
{
	...
	if( strcmp( name + strlen( name ) - 5, ".skin" ))
	{
		...
		skin->surfaces[0]->shader = R_FindShader( name, LIGHTMAP_NONE, qtrue, qtrue, qtrue );
		return hSkin;
	}
	...
	while( text_p && *text_p )
	{
		...
		surf->shader = R_FindShader( token, LIGHTMAP_NONE, qtrue, qtrue, qtrue );
		skin->numSurfaces++;
	}
